...
class ViewGenerator implements GeneratorProvider
{
    ...
    {
        $templateData = $this->commandData->templatesHelper->getTemplate('admin_index.blade', $this->viewsPath);
        ...
        $fileName = 'admin_index.blade.php';

        $path = $this->path.$fileName;

        $this->commandData->fileHelper->writeFile($path, $templateData);
        $this->commandData->commandObj->info('admin_index.blade.php created');
    }

    private function generateAdminTable()
    {
        $templateData = $this->commandData->templatesHelper->getTemplate('admin_table.blade', $this->viewsPath);

        $templateData = GeneratorUtils::fillTemplate($this->commandData->dynamicVars, $templateData);

        $fileName = 'admin_table.blade.php';
        ...
        $this->commandData->fileHelper->writeFile($path, $templateData);
        $this->commandData->commandObj->info('admin_table.blade.php created');
    }

    private function generateAdminShow()
    {
        $templateData = $this->commandData->templatesHelper->getTemplate('admin_show.blade', $this->viewsPath);

        $templateData = GeneratorUtils::fillTemplate($this->commandData->dynamicVars, $templateData);

        $fileName = 'admin_show.blade.php';

        $path = $this->path.$fileName;

        $this->commandData->fileHelper->writeFile($path, $templateData);
        $this->commandData->commandObj->info('admin_show.blade.php created');
    }

    private function generateAdminCreate()
    {
        $templateData = $this->commandData->templatesHelper->getTemplate('admin_create.blade', $this->viewsPath);

        $templateData = GeneratorUtils::fillTemplate($this->commandData->dynamicVars, $templateData);

        $fileName = 'admin_create.blade.php';

        $path = $this->path.$fileName;

        $this->commandData->fileHelper->writeFile($path, $templateData);
        $this->commandData->commandObj->info('admin_create.blade.php created');
    }

    private function generateAdminEdit()
    {
        $templateData = $this->commandData->templatesHelper->getTemplate('admin_edit.blade', $this->viewsPath);

        $templateData = GeneratorUtils::fillTemplate($this->commandData->dynamicVars, $templateData);

        $fileName = 'admin_edit.blade.php';

        $path = $this->path.$fileName;

        $this->commandData->fileHelper->writeFile($path, $templateData);
        $this->commandData->commandObj->info('admin_edit.blade.php created');
    }
}
